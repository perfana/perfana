...
- `SLOW_REQUEST_MS` - **API**: requests slower than this are logged at WARN by `SlowRequestMiddleware` (`apps/api/src/common/middleware/`) as `METHOD url status ms pool=total/idle/waiting jobs=<active worker jobs>` (default `1000`; unset, `''`, `0` or non-numeric all mean the default, never "log everything"). Timed from before the guards, so pool waits inside the API-key lookup and 401/403/429 responses count; `status` is `aborted` when the client hung up first. `waiting > 0` is pool exhaustion, not a slow query; `jobs=` names the BullMQ jobs active at that instant (one shared 2 s snapshot), so a slow-request burst that coincides with a `statistics-calculation` / `control-group-statistics` / `adapt-analysis` job is the heavy aggregation evicting the buffer cache. SSE streams are skipped.
- `SLOW_QUERY_MS` - **API**: TypeORM `maxQueryExecutionTime`; statements slower than this log `slow query (Nms): <sql…200 chars>` at WARN (default `1000`, same fallback rule; TypeORM treats `0` as off, so `0` means the default). The worker pins both its pools to 5000 in code (`apps/worker/src/config/typeorm.config.ts`); `grafana-sync` builds its own TypeORM options and does not get this. The worker also logs one line per job exit — `Job done|soft-failed|failed: <name> (ID: n) <run> in Nms (queued Nms) in <queue>` — so an API slow-request line naming a job can be matched against how long that job ran; a `DelayedError` re-park emits neither, on purpose.
...
- `AGGREGATION_STATEMENT_TIMEOUT_MS` (default `540000`) / `AGGREGATION_WORK_MEM` (default `128MB`) — budget for the **worker's** heavy aggregation transactions (`StatisticsPipeline`, `ControlGroupStatisticsPipeline`), applied by `BasePipelineTypeORM.setAggregationBudget()` as the first statement of the transaction; since v0.2.95.32 also the perf-test aggregates and statistics upsert in `helpers/perf-metrics-writer.ts`, via `withAggregationBudget` (`helpers/aggregation-budget.ts`) in their own transaction. Separate from `ANALYTICS_STATEMENT_TIMEOUT_MS` on purpose: that is a lowerable cap on runaway reads, these two are the job's own work. Keep the timeout strictly below the analytics pool's client-side `query_timeout` (600000, `apps/worker/src/config/typeorm.config.ts`) or node-postgres tears the connection down instead of letting Postgres cancel the statement. `work_mem` is charged per hash/sort node, per parallel worker, and per concurrent job, so the deploy-wide peak is ~this value x (1 + `max_parallel_workers_per_gather`) x 4.
...
`ds_metric_statistics.pct_agg` is the per-run t-digest from #289; `ControlGroupStatisticsPipeline` pools the sketches with `rollup(pct_agg)`. Rows written before #289 — or restored from a backup or SUT transfer — have `pct_agg = NULL`, forcing a raw scan over `ds_metrics` that runs out of time on a large baseline. `ds_control_group_statistics` stays empty and ADAPT reports INSUFFICIENT_DATA against a baseline that is fine.

Since v0.2.93.3 both aggregation transactions run under `AGGREGATION_STATEMENT_TIMEOUT_MS` (540s), not the `ANALYTICS_STATEMENT_TIMEOUT_MS` cap (120s) — `setAggregationBudget()` is their first statement, so the budget covers the whole transaction including `StatisticsPipeline`'s `ramp_up` refresh. Lowering `ANALYTICS_STATEMENT_TIMEOUT_MS` no longer shortens these two jobs.
...
- **Decompress the narrowest span, per run (v0.2.93.3).** `ds_metrics` is compressed with `compress_segmentby = 'test_run_id'`, and `ramp_up` is neither segmentby nor orderby, so an `UPDATE` guarded on it decompresses the run's whole segment as DML even when zero rows change. `findRunsWithStaleRampUpFlags` returns `MIN/MAX(m.time)` over the **disagreeing rows**, and both `decompressChunksForRange` and the `UPDATE` bind those per-run bounds — never the run's full window, never one global min/max across a batch. `decompress_chunk` is chunk-granular and a chunk holds every run in its range, so an over-wide range puts other runs into row store until the compression policy catches up. Per-run statements do not each get their own budget: `max_tuples_decompressed_per_dml_transaction` is per **transaction**. That is why `decompressChunksForRange`'s skip path logs at **warn** — a silent no-op surfaces later as `tuple decompression limit exceeded` with nothing explaining it.
- **The analysis offsets must fit inside the run, guarded in two mirrored places (v0.2.93.3).** The window is `[start + analysisStartOffset, end - analysisEndOffset]`. On a run shorter than the two offsets combined they overlap, every sample matches one of them, and the whole run reads as outside the window — empty `ds_metric_statistics`, an Apdex rollup that misses every transaction, and ADAPT reporting INSUFFICIENT_DATA against a run that has data. The fallback analyses the **whole** run (`duration > ramp_up + ramp_down` guard): offsets are a request to trim, not to discard. The guard exists in `MetricsPipeline` (baked at ingestion) **and** in `RAMP_UP_EXPR` (recomputed on recalculation) — change one, change the other, or a run's flags flip depending on which path last touched it.
- **Do not add a diagnostic `COUNT` back (v0.2.93.3).** Three full `ds_metrics` scans existed only to log "will process N unique metrics" and warn on an expected-vs-actual mismatch: `COUNT(*)` (16 s on 20.6 M rows), a composite `COUNT(DISTINCT ...)` (32 s on the same run; unparallelisable, and still 4.7 s / ~370 MB temp I/O on a 1.58 M-row run), and the control-group twin, which ran **on the fast path too** — the path built to avoid that scan. Only an `EXISTS` probe survives, guarding the `DELETE` so a run whose `ds_metrics` aged out keeps its statistics. Every removed number is already in the `INSERT`'s row count.
