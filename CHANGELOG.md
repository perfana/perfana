...
- **Runs that are shorter than the offsets are now excluded rather than quietly broken.** The existing check only compared the leading offset against the run duration and never looked at the trailing one, so applying a long run's offsets to a short one either forced it to report that no baseline could be found, or silently fell back to analysing the whole run — neither of which was reported as a misconfiguration. The check now uses both offsets together, which is what actually defines the window.
