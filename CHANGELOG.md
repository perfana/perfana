...
### Fixed
- **The worker's chunk decompression and recompression now show up in the production log.** `Decompressing ds_metrics chunk …`, `Decompressed N chunk(s)` and `Recompressed N/N` are logged through NestJS's logger at `log` level, which the worker configured away in production; a nine-minute decompression left no trace at all. Production now logs at `log` level too (a dozen extra lines at boot).
