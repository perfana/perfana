...
### Fixed
- **The worker stopped re-reading a whole live test run every minute, which was the largest single consumer of database I/O on the deployment.** `upsertPerfTestStatistics` recomputes the perf-test `ds_metric_statistics` on every 60 s incremental tick, and it reads the *run*, not the tick — so on a long test its cost climbs with the run while the ticks stay a minute apart, making the total quadratic in run length. Profiled on production with `pg_stat_statements` ordered by blocks read (not by wall clock, which hides it), it was **7055 calls, 5509 GB read and 18.5 hours of database time — eight times the next entry**. That is what collapses the buffer cache while a long test is running — which makes it a candidate for the recurring API stall seen at nightly test end, though not for slow reads logged outside a run window, where `jobs=none` in the `SlowRequest` line rules a worker job out. The pass now runs on a geometric cadence: only once the run has grown by `PERF_TEST_STATS_MIN_GROWTH` (default 30%) of its own length since the last one, which makes the total across a 3-hour run roughly 4.3x a single pass instead of 93x. Nothing a tick writes survives the analysis it belongs to — `StatisticsPipeline` deletes and rewrites every one of these rows from `ds_metrics` at analyze time — so the only thing a skipped tick can affect is how fresh the live display is. Set the variable to `0` to restore the previous behaviour.
- **The virtual-user panel stopped reading three other tests' data to answer about one.** `virtual_users` carried TimescaleDB's default index on its time column, and Postgres preferred it to the `(test_run_id, time)` index on every query — then discarded most of what it read, because four nightly tests share a chunk's seven-day time range and the test id was only applied afterwards. On a 3-hour run the endpoint touched 592,940 buffers to return 261,597 rows; through the right index it touches 11,975. Deployment-wide that index had read 1.65 billion rows. It is dropped (migration 1811). This is not a statistics problem and no amount of `ANALYZE` would have fixed it: each test occupies its own slice of the night, so the two filters are strongly correlated and the planner costs them as if they were independent. Nothing in the product queries virtual users without naming a test run, and the remaining index carries time as its second column, so every existing query keeps a full index path.
- **The analysis time range dialog's overview chart no longer reads millions of raw samples to draw a hundred points.** `GET /test-runs/:id/summary-timeseries` aggregated raw `transactions` and `requests_raw` across the whole run — 5,041,887 rows for ~360 buckets on a 3h07m run, and second on the whole deployment by blocks read (525 calls, 686 GB). It now reads the `transactions_5s` and `requests_raw_5s` continuous aggregates, which already hold the count and mean response time per 5-second bucket, and falls back to the old scan for anything they cannot answer: SUT-imported runs, a system hidden by row-level security, or a deploy whose aggregate refresh policies are not running. Two behaviour changes that are deliberate: the bucket size is rounded to a multiple of 5 seconds so the aggregate composes exactly, and buckets are aligned to the run's 5-second-floored start rather than to its exact start, so they can sit up to 4 seconds earlier than before. Sample totals are identical (verified 5542 = 5542 against the raw query).
...
### Added
- **The Top 10 performance-ranking figure is an ADAPT metric.** The perf-test pipeline writes two new panels on every scenario dashboard — **Transaction Impact** (108) and **Request Impact** (219) — holding `SUM(response_time) / bucket_seconds`, i.e. `avg_rt x throughput`: the `impact_score` the Top 10 lists rank on (`avg_rt x count`) per second of run, so two runs whose bucket sizes differ stay comparable (`SUM(value x bucket)` over the analysis window approximates `test_run_transaction_stats.impact_score`; they differ on NULL response times, which the rollup counts and `SUM` skips, and where the window edge falls inside a bucket). Both carry a classified compare config (`RED_duration`, lower is better, `mean`), so a small response-time shift on a high-volume transaction is judged by the total time it costs the run rather than by its percentage alone. Default 15 % / IQR thresholds apply, and ADAPT's percentage check is mandatory for a full `regression` label — an `absoluteThreshold` alone only yields `partial regression` — so the way to catch a sub-15 % shift on a high-volume transaction is a lower `percentageThreshold` on the Impact panel's compare config (the IQR check, when valid, must agree as well). Rollout caveats: the panels are written by the perf-test stage, which a finalised run **skips** — a run analysed before this version gains them only through a **force re-fetch** re-evaluate, not a plain re-analyse (do not force re-fetch a SUT-imported run that has no raw `requests_raw`/`transactions` — the force path deletes the perf-test rows before it finds nothing to rebuild from); a live run that straddles the worker deploy gets Impact series that start at the deploy (the tail path only aggregates the last tick), and force re-fetch is again the repair; and ADAPT compares Impact against whichever control runs already carry it, so the first runs after the deploy are judged on a 1-, 2-, 3-run baseline for this panel while every other panel uses the full group — expect Impact-driven verdicts to be noisier for the first few runs per workload. `isRequestPanel` (web + report) also accepts 219 so the request-impact rows get their URL; 210-218 stay the web's virtual URL panels, which is why the id is not 210.
...
### Added
- **The API logs every slow request with the two numbers that explain it.** Nothing timed a request before: the global pipeline was guards → audit context → RLS transaction, none of which measured anything, so "the UI freezes and calls are slow" left no trace in the log. `SlowRequestMiddleware` now logs `METHOD url status ms pool=total/idle/waiting jobs=<active worker jobs>` at WARN for anything over `SLOW_REQUEST_MS` (default 1000). It is a middleware rather than an interceptor so the clock starts before the guards — the API-key lookup in `KeycloakEnhancedAuthGuard` waits on the same pg pool, and a 401/403/429 never reaches an interceptor — and it fires on the response's `close`, so the status is the one the client got and a client that hung up first reads `aborted`. `waiting > 0` is pool exhaustion, not a slow query; `jobs=` names the BullMQ jobs active at that instant, so a burst of slow requests that lines up with a `statistics-calculation` / `control-group-statistics` / `adapt-analysis` job is the heavy aggregation evicting the buffer cache, where no single query looks slow. The pool is sampled synchronously at `close`, before the Redis round trip; the job list is one shared 2 s snapshot (`describeActiveJobs` is single-flight), so the burst the line exists to diagnose — or a slowly-read `@Public()` route — costs Redis one `getActive` pair per 2 s, not ~40 commands per response. SSE streams are skipped. `''`, `0`, negative and non-numeric thresholds all mean the default, never "log everything" (`''` is what a compose `${VAR:-}` produces).
- **Every TypeORM pool logs statements slower than a threshold.** `createTypeOrmConfig` sets `maxQueryExecutionTime` and keeps the logger on `warn` in every environment — `logQuerySlow` emits on `warn`, and production was `error` only, so a slow query was measured and dropped. `SLOW_QUERY_MS` on the API (default 1000); the worker pins both its pools to 5000 in code, since its aggregations and `ds_metrics` upsert batches routinely take seconds and 1 s would log every tick. TypeORM treats 0 as off, so 0/NaN/negative fall back to the default rather than silently disabling it. `grafana-sync` builds its own TypeORM options and is unchanged.
- **The worker logs one line per job exit with its cost.** `Job done|soft-failed|failed: <name> (ID) <run> in Nms (queued Nms)` from the factory's `completed`/`failed` listeners, using BullMQ's own `processedOn`/`finishedOn` stamps, so an API slow-request line naming a job can be matched against how long that job actually ran. A `softFail` pipeline that returns `{ status: 'failed' }` is not called "done"; a `DelayedError` re-park emits neither event, so a parked analyze never reads as a failure; a retried attempt has no `finishedOn` and is timed from now.

## [0.2.95.32] - 2026-09-17

### Changed
- **A workload-level Apdex SLO is evaluated in three database round trips instead of ~1,470.** `evaluateWorkloadLevelApdex` ran the full threshold priority chain per transaction (two queries), then the rollup fast path per transaction, each pair wrapped in `SAVEPOINT`/`RELEASE` — on a 294-transaction run about 1,470 statements for one check. It now loads every transaction-specific override for the workload in one query (`transaction_name = ANY(...)`), looks the workload-level threshold up once and only when the benchmark carries none, and answers all transactions from one rollup statement — `unnest(names, thresholds)` joined to `test_run_transaction_stats`, `GROUP BY transaction_name`, so each keeps its own threshold. Only transactions the rollup cannot answer (no row, or a pre-#298 row without `pct_agg_passed`) take the raw `transactions` scan, still under a savepoint each. The one bulk statement runs under its own `SAVEPOINT sp_apdex_rollup`, so a fatal Postgres error in it (the #326 bigint-overflow shape) falls back to per-transaction raw scans instead of failing the benchmark. Names are deduplicated before the call — a transaction that runs in two scenarios must appear once in the `unnest`, or the `GROUP BY` doubles its counts. Per-transaction results are identical to the old statement on `WBS-acceptatie-loadtest_perfana-00003`. Unchanged and now tracked in TODOS.md: a multi-scenario transaction still contributes its name-wide counts once per scenario to the workload totals, exactly as before.
- **The sampler rollup aggregates into a temp table so Postgres can run it in parallel.** `INSERT ... SELECT` cannot use parallel workers, so `SAMPLER_ROLLUP_SQL` sorted every `requests_raw` row of the run serially and spilled even at its 512MB `work_mem` — `Sort Method: external merge Disk: 487,896 kB`, 7.9–8.0 s on a 2.53 M-row run. The same aggregate as `CREATE TEMP TABLE sampler_rollup_base ON COMMIT DROP AS ...` plans with 2 workers (2.65–2.8 s when the scan splits the run's rows evenly, ~6 s when one participant claims most of the run's blocks; `parallel_leader_participation = off` measured slower on the even case and is not set) and the `INSERT` from the 2,236-row temp table is 0.11 s. `last(url_hash, time)` replaces `ARRAY_AGG(url_hash ORDER BY time DESC)[1]`, which materialised every url_hash of a group to keep one. Output checked byte-identical (EXCEPT both ways over the key, both url_hashes and the counts: 0 rows); the statements run under `perfana_system` (TEMP privilege confirmed). `work_mem` is now charged per parallel worker, so the transaction pins `max_parallel_workers_per_gather = 2` and the ceiling is ~1.5 GB per rollup, not 512 MB.
- **The perf-test aggregates and statistics upsert run under the aggregation budget.** `insertDsMetricsFromAggregate` and `upsertPerfTestStatistics` issued bare `dataSource.query` calls at the pool default `work_mem` (4MB) with `statement_timeout` equal to the client `query_timeout` — the requests aggregate spilled 69 MB, the statistics CTE 143 MB, on every live tick. Both now run inside `withAggregationBudget` (own transaction, `set_config(statement_timeout / work_mem, true)` first), the same pair the heavy pipelines use, extracted from `BasePipelineTypeORM.setAggregationBudget` into `helpers/aggregation-budget.ts`. Measured on the 2.5 M-row run: the statistics CTE's outer sort moves in memory (11.6 MB spill gone, 5.99 s → 5.19 s) but its inner `percentile_agg` sort is 314 MB and still spills at 128MB — so the audit's "spills gone" holds for the smaller sorts only. The CLAUDE.md `AGGREGATION_WORK_MEM` budget note now counts these per-live-run statements, which run outside `HeavyStageMutex`. This does not close the TODOS.md "full perf-test collection is not atomic" item: each statement has its own transaction; the DELETE and the four processors are still not one.
- **The full Dynatrace collection writes `ds_metrics` in 200-row batches instead of one statement per point** — `DynatracePipeline.storeMetricsDocuments` issued one `INSERT ... ON CONFLICT` per data point, ~10⁵ round trips per config on a 100-host tenant; it now reshapes the documents (keeping the `timestep`/`ramp_up` DataProcessor computed) and hands them to the incremental collector's `MetricProcessor.upsertMetricsToDatabase`. That upsert now collapses rows sharing a conflict key to one, last wins: a multi-row `INSERT ... ON CONFLICT DO UPDATE` whose `VALUES` carry the same key twice is rejected by Postgres outright (`ON CONFLICT DO UPDATE command cannot affect row a second time`, reproduced on dev), and two enabled `dynatrace_queries` on one panel with the same metric name and no group-by — a shape the dev database already has — produce exactly that pair. The per-row loop tolerated it; the incremental tick path shared the latent bug. Known trade, tracked in TODOS.md: the run's points are flattened into memory beside the documents before the first statement.
- **`ds_panels` has an index on `(test_run_id, application_dashboard_id)`** (migration 1807). Every incremental Grafana tick, `getDsPanelsByTestRun` and the `DELETE` in `PanelsPipeline` sequentially scanned the table (dev: Seq Scan cost 2272 → Bitmap Index Scan cost 5.97). Plain, not `CONCURRENTLY`: a concurrent build cannot run inside the migration transaction, and the table has no retention (one row per panel per analysed run), so on a long-lived deploy the build takes seconds to low tens of seconds and blocks `ds_panels` writers — PanelsPipeline, DynatracePipeline — for that long; reads continue.
- **The tracked-results ADAPT query filters `ds_metric_statistics` by the bound test run ids.** Its `current_metrics` CTE joined on `ms.test_run_id = hr.current_test_run_id`, which is always one of the bound control-group ids, but the planner cannot see that through the CTE chain and scanned the whole table from `historical_regressions`. The redundant `WHERE ms.test_run_id IN (...)` gives it the predicate directly: 70 k rows per worker and 147 ms → 7 k rows and 37 ms on dev; with seq scans discouraged the plan takes `uniq_ds_metric_statistics`, which is what a production-sized table picks on its own.
- **The data sanity check answers "is every point in the ramp-up window?" with an `EXISTS` instead of counting the run.** The "metrics exist but no statistics" branch ran `COUNT(*)` / `COUNT(*) FILTER (WHERE ramp_up)` over every `ds_metrics` row of the run to decide one yes/no; `hasMetrics` has already proved rows exist, so `EXISTS (... ramp_up IS DISTINCT FROM true)` stops at the first steady-state row. `IS DISTINCT FROM true` keeps the old meaning for a never-baked NULL flag (not "in the ramp-up period"). The message loses its data-point count; nothing parsed it.

### Removed
- **The perf-test tick no longer rewrites `grafana_dashboards.panels`.** `updateDashboardPanels` scanned the run's whole `ds_metrics` twice per tick (211 ms parallel seq scan, per minute per live run) to rebuild the `panels` JSON of every dashboard the run wrote to, joined by `uid` unscoped by instance. A perf-test application dashboard has no `grafana_dashboards` row at all (75 on dev, 0 matches); for real Grafana dashboards it duplicated grafana-sync's work, and the artificial Dynatrace rows it also touched have no reader of `panels`. Deleted with its timing step; a test pins that the pipeline never issues a statement against `grafana_dashboards`.
- **`cleanupStaleApplicationDashboards` is no longer called on FK-backed tables.** `ds_panels`, `ds_metric_statistics`, `ds_control_group_statistics`, `ds_adapt_results`, `ds_adapt_tracked_results` and `ds_compare_config` all carry a validated foreign key on `application_dashboard_id`, so the helper's `NOT IN (SELECT id FROM application_dashboards)` can never match — each call was a full sequential scan of the table per job (54 ms / 62 ms on dev at 210 k / 349 k rows, growing with the table) to delete nothing. Removed from PanelsPipeline, DynatracePipeline, StatisticsPipeline, ControlGroupStatisticsPipeline and AdaptPipeline (with its `cleanup-stale-data` substage); ChecksPipeline keeps its call because `check_results` has no such FK.

### Fixed
- Review follow-ups on this branch: the batched-upsert conflict-key dedupe above; mock-only "the row does not echo the name" branches removed from `ApdexCalculator` in favour of fixtures that carry `transaction_name`; the single-transaction rollup wrapper inlined; `SAMPLER_ROLLUP_SQL` references in CLAUDE.md, the test-runs README and `test-runs-performance-query.service.ts` renamed to `SAMPLER_ROLLUP_BASE_SQL`; dead `cleanupStaleApplicationDashboards` spies and queued cleanup answers removed from the Adapt, Dynatrace and ControlGroupStatistics tests (the latter had been feeding the sketch-backfill lookup bogus ids).
- Tests added: bulk rollup failure falls back under the savepoint; workload-level threshold looked up once when the benchmark has none; multi-scenario names deduplicated before the rollup; `applyAggregationBudget` env branches (numeric override, non-numeric → 540000, never `'NaN'`); `upsertPerfTestStatistics` budget order; DynatracePipeline writes N points in one batched statement with the document's `timestep`/`ramp_up`; `MetricProcessor.upsertMetricsToDatabase` conflict-key dedupe and 200-row batching. Worker suite 1,749 passing.
...
- **An SLO with a "not equal" (or "equal", "at most", "at least") requirement now evaluates it.** The worker implemented only `lt` and `gt`; every other operator logged an "unknown operator" warning and passed the check. On `WERKNL-acceptatie-loadtest_perfana-00001` the *Performance test metrics T_WG_Mijn_Vacatures* SLO required `WG_VAC_16_Stuur_Email ≠ 26.25`, the measured value was exactly 26.25, and the result said it met the requirement. All six operators (`lt`, `lte`, `gt`, `gte`, `eq`, `ne`, plus their symbols) are now evaluated in one place shared by the requirement checker and the aggregated benchmark evaluator, and the threshold is compared as a number even though Postgres hands a `NUMERIC` column over as a string — `=` and `≠` could otherwise never match. Existing results are not rewritten; re-evaluate a run to see the corrected verdict.
...
- **A Trend Charts section that selects a whole performance-test dashboard no longer merges the four response-time panels into one row.** The report folded Transaction RT Avg / P90 / P95 / P99 under one display title and kept only the last one's values. Each stored panel is a row of its own, labelled with the stored panel title when several share a name.
- **Applying a saved Trends preset whose dashboard was deleted restores its series again** (it used to stop at "Dashboard not found"); the toast still says the dashboard is gone so the save context is known to be a stand-in.
...
- **The Custom Graphs report section can also draw Trends-card presets.** Each selected trends preset becomes one chart of its series' statistic (the preset's aggregation) per run, over the same window the Trend Charts section uses (since the last ADAPT change point, at most 10 runs), beside the graph presets. The chart values are the same numbers the Trends card shows: the per-run statistic is read from the transaction and sampler rollups, using the preset's own aggregation, so an aggregated series in a report and in the card agree. A template that lists panels and a trends preset that has since been deleted keeps its panel charts with an inline warning instead of an empty section.
- **Generating a report shows where it is.** The HTML generation job records which section it is rendering, `GET /reports/:id` carries it while the report is processing, and the test run page shows a progress panel ("Rendering section 3 of 8: Custom Graphs") until the report is ready — and waits up to 10 minutes rather than giving up on the viewer after 60 s. Several reports generating at once are each tracked. The progress lookup waits at most 300 ms on Redis, so a stalled Redis slows the report request rather than pinning its database connection.
...
### Fixed
- **ADAPT no longer passes a verdict on a metric with a single sample.** A metric needs at least `ADAPT_MIN_SAMPLE_COUNT` data points (default 2) on the test run and on average per control run, or it is labelled `incomparable`; a compare config can override the floor per dashboard, panel or metric via `thresholds.minSampleCount`, and the perf-test scenario panels (which hold one point per run by construction) write `1`. Found on a JMeter run where a sampler whose Transaction Controller parent chain broke in the last seconds of the test landed in `requests_raw` as a bare-named series with one bucket — 759 ms against a control group holding one sample of 436 ms — and was reported as a full `regression` beside the healthy `transaction.sampler` series it duplicated. Replayed on that run the label is now `incomparable` and 270 other one-sample results in the same run go the same way; every real series is unchanged. The listener side of the same defect (writing the detached row at all) is fixed separately in perfana-jmeter-timescaledb.
- **The floor is safe against the config it reads and the rows it counts.** `thresholds.minSampleCount` arrives from the API body under a bare object check, so a non-number is ignored rather than aborting the whole ADAPT batch with a cast error, a fraction is floored, and a zero or negative override is clamped to 1 instead of switching the floor off. A NULL `count` on either side reads as no samples, not as a NULL `control_exists` that would skip the `incomparable` branch and fall through to `no difference`.
- **Migration 1806 backfills `minSampleCount: 1` onto existing scenario-panel configs.** The perf-test pipeline inserts compare configs with `ON CONFLICT DO NOTHING`, so a workload analysed before this version would never have gained the opt-out and its Error Count / Active Threads panels would have turned `incomparable` for good on upgrade. The backfill adds the key once (210 rows on the dev database) without marking their ADAPT results stale, leaves any row that already carries a numeric `minSampleCount` alone, skips a `thresholds` that is not an object rather than corrupting it, runs on API boot so a worker rolled out ahead of it labels those panels `incomparable` on the runs analysed in between (re-evaluate them), and recognises a perf-test dashboard by its metrics source or, for dashboards restored from a SUT export that have none, by the `performance-test-metrics-` uid prefix.
- Two dead SQL builders that still emitted the un-floored `control_exists` (`ControlGroupProcessor.buildControlGroupJoinSQL`, `CompareConfigCache.buildDynamicStatisticsSQL`, no callers) are deleted so nobody can revive the bypass.
...
### Fixed
- **A JMeter-only run no longer deletes and rebuilds its metrics twice over on every analysis.** The orphan sweep that runs first in `analyze-test` compared the perf-test collection-status row against the key `performance_test::null`, but since the April fix for the row explosion the row carries `''`, so the sweep deleted it every time. A run with a Grafana or Dynatrace source beside it still had a row left and skipped collection in about two minutes; a JMeter-only run had none, took the full path, and spent a minute deleting and seven rebuilding what its live ticks had already stored. On 2026-09-13 four large runs finished together and one of them, WERKNL, ran that stage past the ten-minute wall clock beside the other three; the job was recorded as completed with no statistics, no ADAPT and no checks, and its abandoned aggregation ran on until the connection was cut. The key is now built in one place for the whitelist and both sweeps.
- **The perf-test rebuild always runs at analysis time, on every run.** Fixing the sweep exposed why the "skip collection" path must not include it: the per-minute ticks write 1-second buckets and one point per minute for the scenario-level error and thread counts, while the rebuild writes run-sized buckets and one run-total point — which is what every baseline holds. Skipping the rebuild made ADAPT compare errors-per-minute against errors-per-run. Runs with an external source had been taking that path since the previous release and should be re-analysed. The rebuild's delete now preserves the Grafana and Dynatrace rows beside it, refuses to delete when a run has no raw request data to rebuild from (a SUT import without the raw group), and a tick that was still queued when the run completed no longer races it. Coverage no longer counts the perf-test row, which is 100% by construction. Making the ticks write the rebuild's shape, so the rebuild can be skipped, is filed as a follow-up.
- **Every analysis stopped spending three minutes deleting nothing.** `metrics-collection` opened with a delete of `ds_metrics` rows whose dashboard no longer exists, with no run predicate: on a compressed hypertable that is a decompressing scan of every chunk, measured at 175–187 s per analysis before it failed — and the failure was logged without its error text, because the log call had its arguments in the wrong order. The application-dashboard delete already removes those rows in its own transaction, so the sweep is gone. The same sweep on the small result tables also had a `NOT IN` over a nullable column, which one NULL row turns permanently false; it now excludes NULLs.
- **`performance-test-metrics` runs under the heavy-stage lock.** It rebuilds a run from `requests_raw` and then aggregates the whole run's metrics, the same shape as `statistics-calculation`; two of them beside a heavy stage is what pushed WERKNL past the wall clock. It now queues like the other heavy stages instead of racing a timer. The live per-minute tick is unaffected.
...
- **Two worker guards the shorter window makes necessary.** The `missing-data` re-evaluate now decompresses a run's chunks before its gap-fill upsert, as the force re-fetch already did — on a run older than ~3 days that upsert would otherwise stop at `tuple decompression limit exceeded`. And a worker process now keeps at most 8 `ds_metrics` chunks in row store at once (~128 GB at 16 GB/day): a bulk analysis-window apply over 100 runs on distinct days could otherwise inflate the volume by ~1.6 TB. Past the ceiling a run's write stage fails with a warning naming it (`re-evaluate fewer runs at a time`) instead of filling the disk.

## [0.2.95.17] - 2026-09-11

### Fixed
- **Four large analyses finishing together no longer take three of them down.** On 2026-09-11 four runs ended at the same minute; two analyses ran concurrently, each aggregating tens of millions of `ds_metrics` rows on the same Postgres, and evicted each other's pages (buffer cache hit ratio 16 %, 90 MB/s of temp files). Nothing waited on a lock — the jobs were blocking each other on the database itself. Two things were wrong on top of that:
  - The orchestrator raced every stage against a 600 s `setTimeout` that was documented as a pipeline total. When it fired it only abandoned the promise: the aggregation kept running on Postgres while the job reported `partial` (BullMQ *completed*, so the failed count never moved), released its scope lock and its concurrency slot, and the next queued analysis started on top of the orphan.
  - Both analyses' heavy statements then ran to the 540 s `statement_timeout` or the 600 s client `query_timeout`, whichever came first.

  Now the three database-heavy stages — statistics, control-group statistics, ADAPT — run **one at a time across the deployment**, behind a Redis lock (`HeavyStageMutex`: `SET NX PX`, 5 min TTL, 60 s heartbeat, per-acquisition token so a re-dispatched job with the same id cannot release the new holder's lock). The cheap stages still overlap. Those three stages no longer have a wall-clock race at all; Postgres bounds every statement they run. The same lock is taken by the re-evaluate orchestrator's child jobs — by the **child**, never the orchestrator, so a parked analysis can never pin both `perfana-analyze` slots behind a child that cannot be picked up.
- **A second run of the same workload is no longer dropped while the first is analysed.** The scope-lock refusal threw and relied on the job's retry policy (3 attempts inside 15 s); now that a holder's heavy stages queue behind every other analysis, a holder can sit on `sut:env:workload` for an hour, so the second run was failed for good ~15 s after pickup with no progress record ever created. It is now re-parked (`moveToDelayed`, no attempt consumed) a minute at a time for up to 2 h. A heavy-stage lock give-up, or a Redis error inside the acquire, is likewise rethrown into the retry policy instead of being recorded as `partial` — which BullMQ counts as completed.
- **A running heavy stage no longer vanishes from the UI after 5 minutes.** The progress record expires after 5 min and a heavy stage publishes nothing while it runs; the API then evicted the job while its scope lock still refused new analyses. The orchestrator now republishes the record every 60 s for the duration.
- **A re-evaluate no longer fails because its child job sat in a queue.** `waitForJobs` counted queue time against its 600 s budget, so a child behind two analyses was removed and the whole re-evaluate failed. It now polls the child every 10 s and charges the slice to a *running* clock (600 s) or a *parked* clock (1 h) depending on whether the child is in BullMQ's waiting list or active but queued behind the heavy-stage lock. A publish that lands after the wait resolved is dropped rather than leaving the record parked on "Queued".
- **The worker can decompress `ds_metrics` chunks for the first time.** It connects as `perfana_system`, which does not own the hypertables, and TimescaleDB refuses `decompress_chunk` with `must be owner of hypertable`. Every `decompressChunksForRange` since v0.2.93.2 was therefore a silent no-op logged as `skipped`, and the ramp-up `UPDATE` it exists to protect hit `tuple decompression limit exceeded` on any run past 100k rows older than `compress_after` — so an analysis-window change on a run older than a week failed. Migration 1804 adds `SECURITY DEFINER` wrappers (`perfana_decompress_chunk` / `perfana_compress_chunk`), owned by the hypertable owner, `EXECUTE` granted to `perfana_system` only and refused for anything that is not a chunk of the five compressed hypertables. Each decompression runs in its own transaction under a 540 s budget, and a chunk that fails is not retried by the same process — a legacy 7-day chunk is ~10 GB of row store, and every stale run overlapping it would otherwise pay the same failed attempt while holding the lock.

### Added
- **The progress indicator says "Queued".** A job parked behind the heavy-stage lock, or still in BullMQ's waiting list, shows a **Queued** chip with the reason ("waiting for another analysis to finish its database-heavy stage" / "waiting for a free analysis worker") in place of the stage line, in all three variants; the bar and border turn warning-coloured, and the compact variant reads "Queued" instead of "0 %". Jobs not yet picked up are announced every 30 s by `QueuedJobAnnouncer`, which skips a run whose workload already has a running analysis so it never displaces that job's live frame. The holder's job id stays in the worker log; the message users see is neutral, since it would otherwise name another organisation's run.

### Changed
- **`ds_metrics` and `requests_raw` use 1-day chunks from now on** (migration 1804; existing chunks keep their 7-day range until they close). Production had 227 GB of its 228 GB `ds_metrics` in the two open 7-day chunks, at ~16 GB/day of ingest, against 10 compressed chunks totalling 1.15 GB (85.8x). A run's aggregation now reads a ~16 GB chunk instead of ~113 GB, and a decompression converts one day of other runs to row store instead of a week. The cost is chunk count: every hot query filters on `test_run_id` with no time predicate and locks every chunk, so `max_locks_per_transaction` must be raised (256 in `docker-compose.infra.yml`; a deploy running its own Postgres has to do it too, restart required — the migration warns when it is lower, and so does the worker at boot). The same compose sets `client_connection_check_interval=10000` so an aggregation whose worker died is cancelled within seconds instead of running on under the next holder. `compress_after` stays at 7 days until the wrappers are proven on production; shortening it is the step that turns the 227 GB into ~40 GB and is filed in TODOS.md with its preconditions.
...
- **A force re-fetch no longer decompresses whole shared `ds_metrics` chunks to delete one run's rows (#563).** `test_run_id` **is** ds_metrics' `compress_segmentby` column, so a `DELETE` filtered on it alone is segment-targeted and needs no decompression at all. The per-source delete carried one extra predicate — `metrics_source_id IN (...)`, not a segmentby column — which defeats that and forces TimescaleDB to decompress the run's segments as DML. The up-front `decompressChunksForRange` existed only to keep that from hitting `max_tuples_decompressed_per_dml_transaction`, and it paid the same cost: chunks are shared, so it rewrote every *other* run in the same time window too.
  ...
  ~900x faster and ~275x less WAL (11 GB against 41 MB; the ~900x is the time ratio, not the WAL one). `deletePerfTestMetricsForRun` takes that form **always** — no decompressing fallback is left in the force re-fetch at all. When the run holds nothing but perf-test rows it is that bare `DELETE` and nothing else; when it holds anything else, the survivors are copied into a `TEMP TABLE ... ON COMMIT DROP`, the run is deleted wholesale, and they are re-inserted — all three statements in one transaction, so they cannot be lost in between. Measured on a 625,184-row run in a compressed chunk: keep 9,282 rows (115 ms), delete (32 ms), restore (617 ms); Dynatrace rows intact, every perf-test row gone.

  **The survivors are every non-`performance_test` row, regardless of what is being re-collected**, and that is deliberately not a coverage question. Grafana and Dynatrace are external and may no longer hold the window — retention expires, tokens lapse — and Perfana exists to keep those metrics after the source has dropped them. The delete this replaces only ever removed perf-test rows and let the other sources upsert over their own, so a re-collection that returned nothing left the old rows intact; deleting them on the promise of a refetch would destroy the only remaining copy. Perf-test rows carry no such risk — they rebuild from `requests_raw`/`transactions` in the same database, and the old code deleted them unconditionally too. The keep-set averages ~3k rows per run (85k Grafana + 17k Dynatrace across a 14.4M-row table; 9,282 on the run measured above), so copying it is cheap. A row whose `metrics_source_id` is NULL belongs to no source, so nothing would re-collect it either: `getRunMetricsSourceTypes` reports it as `'unknown'` and it is preserved with the rest.

- **Decompression is one chunk per transaction, and the chunks are put back.** `decompressChunksForRange` ran a single `SELECT decompress_chunk(...) FROM timescaledb_information.chunks WHERE ...`, decompressing every matching chunk inside **one** transaction — 267 s and climbing in the original report, pinning the xmin horizon throughout, so none of the ~49M rows the caller then deleted could be vacuumed and unrelated tables sat at 2100% dead tuples. Discovery and decompression are now separate statements, one per chunk. A batch of runs sharing a time window still only decompresses each chunk once, because the `is_compressed` predicate no longer matches a chunk that was already decompressed — the dedupe is that predicate itself, not the tracking set `recompressTouchedChunks` keeps, so there is nothing to go stale against a policy run.

  `recompressTouchedChunks()` puts back exactly what this process decompressed, **once, in the re-evaluate's `finally`, after every stage** — not at the end of the force-refetch stage, and not inside `StatisticsPipeline`. Both of those are wrong: the statistics stage runs next in the same job and `refreshRampUpFlags` needs the same chunks uncompressed, so recompressing between them would make it decompress a second time — and with `REEVALUATE_CHUNK_SIZE` splitting a batch into several statistics jobs, once per chunk of runs. At the 153 s per decompression measured here that turns the saving into a regression. Best-effort by contract: the caller has already committed. Recompression was previously left to the columnstore policy, which during the original measurement had not run for 10.5 h — until it does, every query over those chunks scans row store.

- **The up-front decompression is kept on the path that still needs it.** Removing it wholesale was wrong: it guarded two statements, not one. Grafana and Dynatrace re-collect with `INSERT ... ON CONFLICT DO UPDATE` (`metric-processor.ts`, `DynatracePipeline.ts`), which decompresses matching segments as DML. When perf-test is not among the re-collected sources no delete runs, so nothing puts the run's rows into row store first, and a run over 100k rows in that segment would have died on `tuple decompression limit exceeded` — swallowed by the per-source catch, leaving a green job and stale data. `decompressChunksForRange` therefore still runs on that branch; the delete branch does not need it, because delete-then-restore already leaves the rows uncompressed.

- **The preserve/restore transaction runs at `REPEATABLE READ`.** The `CREATE TEMP TABLE ... AS SELECT` and the `DELETE` are separate statements, so at the default READ COMMITTED they take separate snapshots and a row committed between them would be deleted without ever being copied. Narrowing the `DELETE` to spare it is not available — that is the non-segmentby predicate the method exists to avoid — so one snapshot is what turns silent loss into a serialization error.

- **`StatisticsPipeline` is explicitly ruled out of the segment-targeting fix.** Its `refreshRampUpFlags` guards on `ramp_up`, which is neither segmentby nor orderby, so unlike the force-refetch delete it genuinely cannot avoid decompression. It already narrowed to each run's disagreeing-row span (v0.2.93.3) and now gets the per-chunk transactions; it deliberately does **not** recompress itself, for the reason above.
...
### Fixed
- **An analysis that blows up now fails its job instead of being recorded as completed.** `simple-workers.ts` wraps every processor as `return await processor(job)`, so `analyze.ts`'s catch-all returning `{ status: 'failed' }` **resolved** that promise and BullMQ marked the job **completed** — no retry, no entry in the failed set, nothing for an operator to find. A run whose ten-stage pipeline threw looked exactly like one that succeeded.

  It also meant the retry policy this job type has always carried never once fired: `attempts: 3` with exponential backoff from 5 s, agreed on by both `SIMPLE_JOB_OPTIONS['analyze-test']` and the `perfana-analyze` queue default. Throwing activates it. Re-running is safe — every stage deletes and rewrites its own rows — and the `sut:env:workload` scope lock is released in the `finally` before BullMQ reschedules. That release is what the retry depends on, so it is now pinned by its own test: without it every attempt would hit the scope-lock refusal branch, which also throws, and the job would burn all three attempts without re-running a single stage.

  The original error instance propagates rather than a re-wrapped message, so the failed-set entry carries the real stack. A thrown non-Error is converted to an `Error` first, because that is what BullMQ records. Invalid job data throws too: it will never succeed on retry, but recording it as completed hides it entirely.

  The `partial` return above the catch-all is deliberately untouched — that is the orchestrator reporting a stage failure under `errorHandling: 'abort'`, a readable outcome rather than an unhandled exception.
...
- **Re-evaluating several test runs at once no longer stalls and then gives up.** The previous two releases each removed one reason the baseline step ran out of time; this one removes the rest, found by reading the logs from a batch that still failed after upgrading.
  ...
  Separately, a test run **shorter than its own analysis window** was being emptied. The analysis range trims a warm-up period from the start and a wind-down from the end; nothing checked that those two trims fit inside the run. When they do not — a short run against offsets configured for a long one — they overlap, and every measurement falls inside one or the other, so the whole run is treated as outside the window. The consequences all read as missing data rather than as a misconfiguration: no statistics, an Apdex that falls back to the slow path and reports nothing for most transactions, and ADAPT concluding it has insufficient data for a run that plainly has some. It also made the ramp-up refresh described below rewrite every measurement in the run instead of a narrow band. Perfana now analyses the whole run when the offsets do not fit, on both the ingestion path and the recalculation path.
...
### Fixed
- **ADAPT no longer dead-ends on a baseline whose statistics were built by an older version.** Opening a test run could show "ADAPT requires valid baseline data. The 1 control run contained insufficient metrics — they may have been too short or aborted", pointing you at running another full-duration test. On a baseline that ran a full hour and holds over a million data points, every part of that was wrong, and neither Re-evaluate nor Re-fetch could ever change it. The real cause was that the baseline's stored statistics were missing a summary that a later version began saving, so building the comparison figures fell back to reading every raw data point and gave up after two minutes. Perfana now rebuilds that missing summary from the data it already holds — seconds of work, no re-fetching from Grafana or Dynatrace — before building the comparison, so the run reaches a real verdict on its own.
- **The ADAPT message no longer blames a baseline that is fine.** When comparison figures cannot be built, Perfana now checks whether the baseline actually has statistics before deciding what to say. If it does, the message names the real cause and offers the action that fixes it, instead of asserting the test was too short or aborted.
- **A failed baseline-aggregation step is reported instead of passed over.** The step could fail while still being recorded as completed, so the run walked on into ADAPT with nothing to compare against and the log showed a tick. It is now reported as failed and ADAPT does not run on an empty baseline.
- **The statistics log no longer reports deleting zero records when it deleted many.** The count was read from the wrong place and was always zero.

### Added
- **Recalculate baseline statistics.** When ADAPT reports that it could not build a baseline, a button now appears next to that message to repair it. It recomputes the statistics for the baseline runs — the runs that actually need it, not the one you are looking at — from measurements already stored, so it fetches nothing from Grafana or Dynatrace and works on old runs whose dashboards no longer cover the time window. The button appears only for that one cause; the other reasons ADAPT can decline have no remedy it could apply.

## [0.2.89.1] - 2026-08-30

### Fixed
- **The 409 a delete can return is now described in the API docs.** It was returned but never declared, so `/api/docs` listed only success and not-found for that endpoint.
...
- The measurement columns beside a URL keep their width. A table that already assigns every column a percentage — the two top-10 lists — now takes only the half of the fix that stops the blow-out, because the other half competed with those percentages and squeezed the response-time column. In the tables that size themselves, the headers no longer wrap: "Avg Response (ms)" was answering the squeeze by stacking one word per line, which turned the header of the error breakdown and the Apdex requests table into a three-line block.
...
### Fixed
- **Opening the Service Level Objectives card no longer freezes the page.** On a test run whose Apdex SLO covers a few hundred transactions, clicking the collapsed card locked the browser for a second or two before it opened, and the Environment Configuration card did the same. Neither was waiting on the server: both cards had already built their entire contents while still collapsed — around 14,000 hidden elements for the SLO card and 4,800 for Environment Configuration — and the click was what finally forced the browser to lay all of it out. The cost was not even contained to the card you clicked, because expanding one forced the other's hidden contents to be recalculated too. Both cards now build their contents when you open them, and the two long tables inside them render only the rows near your screen rather than all of them at once. Measured on a run with 292 transactions and 218 configuration changes: the page starts with 1,342 elements instead of 20,112, opening the SLO card no longer blocks the browser at all, opening a large Apdex SLO went from 561ms to 62ms, and Environment Configuration opens in 87ms. Scrolling, sorting, filtering and expanding an individual transaction are unchanged. Opening one card while another is already open keeps both tables showing the right rows — a card expanding above a long table slides it down the page, and the table now re-measures its position instead of leaving a blank stretch where the rows should be.
...
- **The Add Dynatrace Entity dialog no longer jumps while options load.** Host and tag options come from Dynatrace and can take seconds to arrive; the host list grew into its content on arrival, and because the dialog was vertically centred that moved every control — including the ones above it — right as you were reaching for one. The list now reserves its height from the moment you pick Hosts, and the dialog is anchored to the top so anything that does grow only pushes downward. The tag and value filters are held shut while a fetch is in flight, with a spinner saying so, since their options are replaced wholesale when it lands and an open list that gets swapped under the pointer is the other way this dialog collected misclicks.
...
- **Audit retention actually deletes something now.** Removing expired records meant dropping a month's table, which needs table ownership the worker likewise does not have — so `AUDIT_RETENTION_MONTHS` (default 24) had never once taken effect. The nightly job is now a delete of rows past the retention window, which the worker's role is permitted to run, and it logs how many rows it removed instead of failing quietly. It works in batches of 10,000 so the first run after this upgrade — which has two years of never-deleted records to get through — cannot hold a single long lock, and it no longer runs before the worker starts picking up jobs. A retention window that is not a positive number is refused with a warning rather than used: `AUDIT_RETENTION_MONTHS=0` would have meant "older than now", which is the whole audit trail.
- **Audit records could be read by users who were not allowed to see them.** Access rules were attached to the parent `audit_logs` table only, so querying a monthly partition by name returned rows that querying `audit_logs` correctly hid. Every partition now enforces the parent's rules, and the pre-ship check that had skipped partitions entirely — which is why it never caught this — now asserts the property directly: every partition of `audit_logs` enforces row-level security, carries no rules of its own, and at least one of them accepts any date.
...
### Removed
- **387 lines of dead chart code in the Trends card.** `trends-chart-utils.ts` exported eight functions — trace building, layout, config, changepoint markers — that nothing imported; `useTrendsPlot` reimplements all of it inline. Only a barrel re-export kept the file reachable, which is why the dead-code check never reported it. Deleted along with three other unused helpers. No behaviour change.
...
### Fixed
- **Removed the automatic organization backfill that 0.2.68.11 added.** It repaired a condition nobody had reported, and on an installation with real history it would have rewritten every metrics table with a missing organization before the API accepted its first request — a long start-up holding locks the whole time. Upgrading now runs one statement: adding the missing column from 0.2.68.12. The organization repair is still available, deliberately and in batches, through `docs/ops/2026-08-21-org-id-backfill-runbook.md`.
- **Upgrading can no longer stall on a busy database.** Schema changes run while the worker and dashboard sync are live, and the database waits for its lock forever by default — so an upgrade could sit there indefinitely while everything else queued behind it. The one remaining change now waits briefly, retries, and reports a clear failure naming the query to look for rather than hanging.
...
### Added
- **Expanding a transaction now shows the shape of the test plan that produced it, not just a flat list.** Requests that ran inside a loop, a conditional, or a sub-transaction are clustered under a band naming it, nested where the plan nests them. Each band reports what its controller can honestly answer: a parallel group reports how long the concurrent pass itself took, while a loop or a conditional reports counts only — a loop has no measured duration, so its response-time columns stay empty rather than carrying a number nothing produced. This explains the count differences that previously looked arbitrary: a request inside a loop has several times the count of its neighbour because it repeats, and a request behind a condition has fewer because it sometimes did not run. The Thread Group and the transaction you expanded are left out, since the card already names them — so a transaction whose requests share one plain path renders exactly as it did before.
- **A parallel group says what it saved**, for runs recorded while the load test tool still reported per-execution timings. Alongside the group's own duration, the band reports what the same requests would have cost one after another and the resulting ratio — "114.00 ms if serial · 2.6× saved" — which is the comparison the individual response times cannot show. Requests that fire more than once per execution are weighted accordingly, and no saving is claimed when the requests barely overlapped.
- **The Performance Analysis card shows which requests a virtual user issued at the same time.** Expanding a transaction used to list every request in a flat table, so a request that ran concurrently with its neighbours looked identical to one that ran after them — the numbers simply did not add up to the transaction's duration and nothing explained why. Requests that ran together are now clustered under a band naming their parallel group, and the band reports how long the group itself took — average, 95th and 99th percentile, and how many executions passed or failed — in the same columns as the requests beneath it. That duration is measured per execution as last finish minus first start, so it is the time a user actually waited, not the sum of the concurrent requests. Percentiles computed from very few executions are marked as such rather than presented as if they carried the same weight as a percentile over thousands of requests. A group's duration is measured across its executions when the run is analysed, so while a test is still running the band names the group and says the timings follow. Transactions with no concurrent requests render exactly as before.

### Changed
- Request records now carry where they sit in the test plan (`requests_raw.source_element_path`), populated by load test tools that report it. Each entry names an element, its type, and which of several identically-named siblings it is, ending at the request itself. Runs recorded before this, and tools that do not report it, leave the field empty and are displayed as they always were. Nothing needs switching on: the tool supplies the metadata when the column exists, so an empty band is answered by checking the column, not by hunting for a setting.
- A request's position is now recorded rather than what the test plan was doing at the moment it ran, following the load test tool's own change of approach. The plan structure this drives is unaffected — it only ever used an element's name and type. What is no longer expressible is which loop pass or which concurrent execution a request belonged to.

### Removed
- **Parallel groups no longer report their own duration for newly recorded runs.** Measuring how long a concurrent pass took requires knowing which requests belonged to the same pass, and the load test tool no longer marks that. Such groups still cluster correctly and still say they ran concurrently; they report no average, percentiles, execution count, or "saved by concurrency" figure, and the band says so plainly rather than implying the numbers are still being computed. Runs already recorded keep their timings in full — the analysis that produces them still runs, and simply finds nothing to do on newer runs.
...
### Fixed
- **Report PDFs no longer silently drop the right-hand columns of a wide table.** The Apdex transaction table's rightmost column (the rating pill) was cut off mid-word at the page edge — `UNACCEPTA…`, `EXCELL…` — so the rating never made it into the document at all. Two things caused it. The pill is an `inline-block` whose text cannot break, so it set a min-content width wider than the page; and the content column was only ~140mm of a 210mm A4 page, because the print stylesheet's `body { padding: 20mm }` stacked on top of the 15/20mm margins Puppeteer already applies. Data cells now wrap anywhere (headers still break on spaces, so they read normally), and the print padding is gone — Puppeteer owns the page margins.
- **Report PDFs no longer waste roughly 40% of every page, or emit a near-blank one.** `page-break-inside: avoid` was set on every `section` and on whole tables, so any card that did not fit in the remaining space jumped to a fresh page and left the rest of the previous one empty. Breaks now happen at table-row granularity, with the table header repeating on each page. Separately, the cover page's `min-height: 100vh` plus the doubled body padding overflowed the page box by exactly the padding, producing a second page containing nothing but a sliver of the cover gradient. On a representative 7-page report this is now 6 pages with no blank page and no half-empty ones.
- **The report header and footer are no longer printed twice.** The page counter appeared as both "Page 1 of 7" and "Page 1 of 7 | Generated by Perfana" on every page: Chrome 142 honours the `@page { @bottom-right }` counter, which duplicated the footer Puppeteer already draws. The `@page` rule is gone; Puppeteer owns headers, footers and margins. The in-document `.page-header` is hidden in print for the opposite reason — Chrome does not implement `position: running()`, so instead of moving into the top page margin it printed inline on page 1, directly beneath the real header. The generation timestamp it carried is still in the report footer.
- **An empty text block no longer prints as a bare bordered card.** A `text_block` section with no content and no comment rendered an empty `<section>`, which showed up in the PDF as a stray empty box between real sections. It now renders nothing; a text block carrying only a comment still renders.
