...
### Added
- **Recalculate statistics.** A new action on both the test-run list and the run detail page recomputes a run's metric statistics from the measurements already stored. It fetches nothing from Grafana or Dynatrace, so it works on old runs whose dashboards no longer cover the time window. Use it on a baseline run when ADAPT reports it could not build a baseline.
