...
- **Migration 1806 backfills `minSampleCount: 1` onto existing scenario-panel configs.** The perf-test pipeline inserts compare configs with `ON CONFLICT DO NOTHING`, so a workload analysed before this version would never have gained the opt-out and its Error Count / Active Threads panels would have turned `incomparable` for good on upgrade. The backfill adds the key once (210 rows on the dev database), leaves any row that already carries a `minSampleCount` alone, and recognises a perf-test dashboard by its metrics source or, for dashboards restored from a SUT export that have none, by the `performance-test-metrics-` uid prefix.
