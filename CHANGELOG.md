...
- **A parallel group now says what it saved.** Alongside the group's own duration, the band reports what the same requests would have cost one after another and the resulting ratio — "114.00 ms if serial · 2.6× saved" — which is the comparison the individual response times cannot show. Requests that fire more than once per execution are weighted accordingly, and no saving is claimed when the requests barely overlapped.
...
### Changed
- Request records now carry the chain of controllers they ran under (`requests_raw.parent_controllers`), populated by load test tools that report it. Each entry names a controller, its type and the pass it was on; the parallel group is the innermost entry that is a Parallel Controller, and that entry identifies the concurrent pass. Runs recorded before this, and tools that do not report it, leave the field empty and are displayed as they always were.
## [0.2.63.1] - 2026-08-15
