...
6. **`ds_metrics` is compressed with `compress_segmentby = 'test_run_id'`, so a per-group lookup on it is a trap.** A compressed chunk holds one segment per run; nothing below `test_run_id` — `application_dashboard_id`, `panel_id`, `metric_name` — can be pushed into it. Any query that visits `ds_metrics` once per metric therefore decompresses the whole run's segment once per metric. `StatisticsPipeline` fetched `last_value` through exactly such a `LEFT JOIN LATERAL`; on an uncompressed chunk it rode the `uniq_ds_metrics_upsert` btree and was instant, on a compressed one it was 60.1 s for 12,370 groups (v0.2.93.2 replaced it with TimescaleDB's `last(value, time)` in the pass already running: 1.2 s, identical output). Compression kicks in after 7 days, and every run the sketch backfill visits is older than that, so a dev database — where nothing is compressed yet — will never show this. Aggregate in the single pass; if you genuinely need a per-group probe, decompress the range first.
