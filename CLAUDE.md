...
3. **A dashboard `uid` is unique only within a Grafana instance.** The same uid routinely exists on several, so a lookup by uid should also scope by `grafana_instance_id`. Not yet true everywhere: `GrafanaDashboardsService.remove`'s delete pre-check matches `dashboardUid` unscoped, so a same-uid application dashboard on another instance can trigger a false 409.
