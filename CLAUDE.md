...
6. **ADAPT says it could not build a baseline / INSUFFICIENT_DATA on a healthy baseline** → the baseline's `ds_metric_statistics` rows are missing `pct_agg` and the control-group aggregation timed out. Run "Recalculate statistics" on the **control run**, then re-evaluate. See "ADAPT's baseline depends on the `pct_agg` sketch" above.
