...
- **The re-evaluate orchestrator's `waitForJobs` no longer counts parked time.** It polls the
  child every 10 s and charges the slice to a running clock (`JOB_WAIT_TIMEOUT_MS`, 30 min since v0.2.95.18 — the child now decompresses for real, up to 540 s per chunk, before it aggregates) or a
