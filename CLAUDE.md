...
**Not every 409 is this one.** A constraint violation the user has to act on is also a 409 and
returns nothing — `POST`/`PUT /benchmarks` answer `uq_benchmarks_active_metric_target` that way
(v0.2.96.15). Say which kind a new 409 is in its `@ApiResponse` description, and see "Turning a
constraint violation into a 409" in [apps/api/CODING_RULES.md](apps/api/CODING_RULES.md) for the
three things that go wrong when raising one.
