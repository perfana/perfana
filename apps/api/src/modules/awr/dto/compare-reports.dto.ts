...
import type { ChangeDirection, SqlComparisonResult, WaitEventComparisonResult, LoadProfileComparisonResult, SeveritySummary } from '../types/analysis.types';
...
/**
 * SQL statement comparison response item
 */
export class SqlComparisonItemDto {
  @ApiProperty({ description: 'SQL ID', example: 'abc123xyz' })
  sqlId!: string;

  @ApiPropertyOptional({ description: 'SQL text (truncated)', example: 'SELECT * FROM users...' })
  sqlText?: string;
  ...
  @ApiProperty({
    ...
    example: { elapsedTime: 10.5, cpuTime: 8.2, bufferGets: 1000 },
  })
  current!: {
    elapsedTime?: number;
    cpuTime?: number;
  ...
  @ApiPropertyOptional({
    ...
    example: { elapsedTime: 8.0, cpuTime: 6.5, bufferGets: 800 },
  })
  baseline?: {
    elapsedTime?: number;
    cpuTime?: number;
  ...
  @ApiProperty({
    ...
    example: { elapsedTime: 31.25, cpuTime: 26.15, bufferGets: 25.0 },
  })
  deltaPercent!: {
    elapsedTime?: number;
    cpuTime?: number;
  ...
  @ApiProperty({ description: 'Whether this is a significant regression', example: true })
  isSignificantRegression!: boolean;

  @ApiProperty({ description: 'Whether this is a significant improvement', example: false })
  isSignificantImprovement!: boolean;
}

/**
 * Wait event comparison response item
 */
export class WaitEventComparisonItemDto {
  @ApiProperty({ description: 'Wait event name', example: 'db file sequential read' })
  event!: string;

  @ApiPropertyOptional({ description: 'Wait class', example: 'User I/O' })
  waitClass?: string;
  ...
  @ApiProperty({
    description: 'Current values',
    example: { totalWaitTime: 120.5, waits: 5000, avgWaitMs: 24.1 },
  })
  current!: {
  ...
  @ApiPropertyOptional({
    description: 'Baseline values',
    example: { totalWaitTime: 100.0, waits: 4000, avgWaitMs: 25.0 },
  })
  baseline?: {
    totalWaitTime?: number;
    waits?: number;
  ...
  @ApiProperty({
    description: 'Percentage changes',
    example: { totalWaitTime: 20.5, waits: 25.0, avgWaitMs: -3.6 },
  })
  deltaPercent!: {
    totalWaitTime?: number;
    waits?: number;
 ...
 */
export class LoadProfileComparisonItemDto {
  @ApiProperty({ description: 'Metric name', example: 'DB Time' })
  name!: string;

  @ApiProperty({ description: 'Current value', example: 125.5 })
  currentValue!: number;

  @ApiProperty({ description: 'Baseline value', example: 100.0 })
  baselineValue!: number;

  @ApiProperty({ description: 'Absolute change', example: 25.5 })
  delta!: number;

  @ApiProperty({ description: 'Percentage change', example: 25.5 })
  deltaPercent!: number;
  ...
  @ApiPropertyOptional({ description: 'Unit of measurement', example: 's' })
  unit?: string;
}
