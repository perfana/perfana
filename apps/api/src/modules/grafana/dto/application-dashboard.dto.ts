...
// Additional DTOs for dashboard operations
export class DashboardVariableValuesDto {
  @ApiProperty({ description: 'Variable name' })
  @IsString()
  variableName!: string;

  @ApiProperty({ description: 'Possible values for the variable', type: [String] })
  @IsArray()
  @IsString({ each: true })
  possibleValues!: string[];

  @ApiProperty({ description: 'Selected values for the variable', type: [String] })
  @IsArray()
  @IsString({ each: true })
  selectedValues!: string[];
}

export class DashboardRenderRequestDto {
  @ApiProperty({ description: 'Application dashboard ID to render' })
  @IsUUID()
  applicationDashboardId!: string;

  @ApiPropertyOptional({ description: 'Variable overrides for rendering' })
  @IsOptional()
  @IsOptional()
  variables?: Record<string, string[]>;

  @ApiPropertyOptional({ description: 'Time range for the dashboard' })
  @IsOptional()
  @IsOptional()
  timeRange?: {
    from: string;
  ...
  @ApiPropertyOptional({ description: 'Snapshot timeout override (seconds)' })
  @IsOptional()
  @IsNumber()
  @Min(1)
  @Max(300)
