...
export interface CreateTestRunData {
  ...
  /** Analysis end offset in seconds (trailing ramp-down period excluded from analysis) */
  ...
  updatedBy?: string;
  /** ADAPT mode: DEFAULT (regression) or BASELINE (always accepted) */
  adaptMode?: string;
}
    ...
    return new CreateTestRunCommand(
      {
        ...
        analysisEndOffset: params.analysisEndOffset ?? 0,
