...
| Rethrowing a unique-violation from `insertBenchmarkBasedOnProfileBenchmark` | A profile SLO can land on a panel that already carries an enabled **manual** SLO. The existence probe keys on `generic_check_id`, which a manual SLO never has, so only `uq_benchmarks_active_metric_target` catches the overlap (migration 1812, v0.2.96.15). Rethrowing aborts provisioning for the whole dashboard over one collision — the v0.2.89.0 "one rejected dashboard aborts the sweep" shape. Detect it with `isDuplicateSloTargetError` from `@perfana/shared/utils/duplicate-slo-target` (never a local copy of the index name), return `skippedDuplicateTarget: true`, and log which panel and what the operator should do about it |
