...
function parseAggregatedMetricSource(metricName: string, sourceType?: string | null) {
...
describe('parseAggregatedMetricSource', () => {
  describe('returns undefined when source_type is not performance_test', () => {
    it('returns undefined for null source_type', () => {
      expect(parseAggregatedMetricSource('transactions.login.response_time.p95', null)).toBeUndefined();
    });

    it('returns undefined for undefined source_type', () => {
      expect(parseAggregatedMetricSource('transactions.login.response_time.p95', undefined)).toBeUndefined();
    });

    it('returns undefined for unrelated source_type', () => {
      expect(parseAggregatedMetricSource('transactions.login.response_time.p95', 'grafana')).toBeUndefined();
    });
  });

  describe('returns undefined for malformed metric names', () => {
    it('returns undefined for metric with fewer than 3 parts', () => {
      expect(parseAggregatedMetricSource('transactions.response_time', 'performance_test')).toBeUndefined();
    });

    it('returns undefined for unknown base name', () => {
      expect(parseAggregatedMetricSource('transactions.login.cpu_usage.p95', 'performance_test')).toBeUndefined();
    });

    it('returns undefined for unknown type with response_time', () => {
      expect(parseAggregatedMetricSource('services.login.response_time.p95', 'performance_test')).toBeUndefined();
    });
  });

  describe('transactions.*.response_time mappings', () => {
    it('maps transactions + p95 stat', () => {
      expect(parseAggregatedMetricSource('transactions.login.response_time.p95', 'performance_test'))
        .toEqual({ metric: 'transaction_response_time', stat: 'p95' });
    });

    it('maps transactions + avg stat', () => {
      expect(parseAggregatedMetricSource('transactions.login.response_time.avg', 'performance_test'))
        .toEqual({ metric: 'transaction_response_time', stat: 'avg' });
    });

    it('maps transactions without explicit stat (falls back to avg)', () => {
      expect(parseAggregatedMetricSource('transactions.login.response_time', 'performance_test'))
        .toEqual({ metric: 'transaction_response_time', stat: 'avg' });
    });
    ...
    it('returns undefined when last part is not a VALID_STAT and not response_time', () => {
      ...
      expect(parseAggregatedMetricSource('transactions.login.response_time.sum', 'performance_test'))
        .toBeUndefined();
    });
  });
  ...
  describe('requests.*.response_time mappings', () => {
    it('maps requests + p99 stat', () => {
      expect(parseAggregatedMetricSource('requests.api.response_time.p99', 'performance_test'))
        .toEqual({ metric: 'request_response_time', stat: 'p99' });
    });

    it('maps requests without explicit stat (falls back to avg)', () => {
      expect(parseAggregatedMetricSource('requests.api.response_time', 'performance_test'))
  ...
  describe('error_rate mapping', () => {
    it('maps error_rate to error_percentage with stat avg regardless of input stat', () => {
      expect(parseAggregatedMetricSource('transactions.login.error_rate.p95', 'performance_test'))
        .toEqual({ metric: 'error_percentage', stat: 'avg' });
    });

    it('maps error_rate without explicit stat', () => {
      expect(parseAggregatedMetricSource('transactions.login.error_rate', 'performance_test'))
        .toEqual({ metric: 'error_percentage', stat: 'avg' });
    });
  });
});
