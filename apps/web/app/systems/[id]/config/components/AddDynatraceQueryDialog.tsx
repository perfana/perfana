...
interface AddDynatraceQueryDialogProps {
  open: boolean;
  onClose: () => void;
  onAdd: (data: CreateDynatraceQueryDto) => void;
  systemId: string;
  environment: string;
  workload: string;
  loading?: boolean;
...
export default function AddDynatraceQueryDialog({
  ...
  const [formData, setFormData] = useState<CreateDynatraceQueryDto>({
    dynatraceConfigId: '',
  ...
  // Update form data when systemId, environment, or workload changes
  useEffect(() => {
    setFormData(prev => ({
      ...prev,
      systemUnderTestId: systemId,
  ...
  const handleSubmit = () => {
    ...
    const submitData: CreateDynatraceQueryDto = {
      ...formData,
