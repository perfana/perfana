...
import { DeepLink, VariableDefinition, BASE_VARIABLES } from '../types';

interface UseEditDeepLinkProps {
  open: boolean;
  deepLink: DeepLink | null;
  systemName: string;
  onSubmit: (id: string, deepLink: {
...
interface UseEditDeepLinkReturn {
  // Form state
  name: string;
...
export function useEditDeepLink({
  ...
  onSubmit,
  onClose,
}: UseEditDeepLinkProps): UseEditDeepLinkReturn {
  // Form state
  ...
  // Reset form when dialog opens/closes or deepLink changes
  useEffect(() => {
    if (open && deepLink) {
      setName(deepLink.name);
      setUrl(deepLink.url);
      setTags(deepLink.tags ?? []);
      setNameError(null);
      setUrlError(null);
      setSubmitting(false);
      resetAutocomplete();
  ...
  const fetchLatestConfigKeys = useCallback(async () => {
    if (!deepLink?.testEnvironment || !deepLink?.workload || !systemName) return;

    try {
      setLoadingConfigKeys(true);

      const response = await authenticatedFetch(
        `/test-runs/config-keys/latest?system=${encodeURIComponent(systemName)}&environment=${encodeURIComponent(deepLink.testEnvironment)}&workload=${encodeURIComponent(deepLink.workload)}`,
        {
          method: 'GET',
          headers: {
  ...
  const handleSubmit = useCallback(async () => {
    if (!validateForm() || !deepLink) return;

    try {
      setSubmitting(true);
      await onSubmit(deepLink.id, {
        systemUnderTestId: deepLink.systemUnderTestId,
        testEnvironment: deepLink.testEnvironment,
        workload: deepLink.workload,
        name: name.trim(),
        url: url.trim(),
        tags,
      });
  ...
  }, [validateForm, deepLink, onSubmit, name, url, tags, onClose]);
