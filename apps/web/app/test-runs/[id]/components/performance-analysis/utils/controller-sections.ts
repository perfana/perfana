 ...
 * Thread Group, which is the same for every row in the run; the Transaction Controller the table
 * is already headed by; and every `other` controller — Module, Test Fragment, Simple Controller
 * and friends only organise the test plan, they say nothing about how the requests underneath
 * them executed, and a real plan nests three or four of them above every row. What survives is
 * the structure that actually differs between the requests on screen.
 *
