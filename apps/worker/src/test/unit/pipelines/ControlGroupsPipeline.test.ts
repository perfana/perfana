...
describe('ControlGroupsPipeline', () => {
  ...
  describe('SCALING mode - findControlTestRuns', () => {
    it('should select explicit baseline run when baselineTestRunId is set', async () => {
      const testRun = {
        test_run_id: 'scaling-run-3',
        system_under_test_id: 'sut-1',
        workload: 'load-test',
        test_environment: 'production',
        start_time: new Date('2024-01-01T12:00:00Z'),
        adapt_config: { mode: 'SCALING', baselineTestRunId: 'baseline-run-1' },
      };

      mockManager.query.mockResolvedValue([
        { test_run_id: 'baseline-run-1' },
      ]);

      const result = await (pipeline as any).findControlTestRuns(mockManager, testRun, null);

      expect(result).toEqual(['baseline-run-1']);
      expect(mockManager.query).toHaveBeenCalledWith(
        expect.stringContaining('test_run_id = $1'),
        ['baseline-run-1']
      );
      // Should NOT include tuple filters
      expect(mockManager.query).not.toHaveBeenCalledWith(
        expect.stringContaining('system_under_test_id'),
        expect.any(Array)
      );
    });

    it('should return empty when explicit baseline not found', async () => {
      const testRun = {
        test_run_id: 'scaling-run-3',
        system_under_test_id: 'sut-1',
        workload: 'load-test',
        test_environment: 'production',
        start_time: new Date('2024-01-01T12:00:00Z'),
        adapt_config: { mode: 'SCALING', baselineTestRunId: 'nonexistent-run' },
      };
    ...
    it('should select most recent prior run when no baselineTestRunId', async () => {
      const testRun = {
        test_run_id: 'scaling-run-2',
        system_under_test_id: 'sut-1',
        workload: 'load-test',
        test_environment: 'production',
        start_time: new Date('2024-01-01T11:00:00Z'),
        adapt_config: { mode: 'SCALING' },
      };

      mockManager.query.mockResolvedValue([
        { test_run_id: 'scaling-run-1' },
      ]);

      const result = await (pipeline as any).findControlTestRuns(mockManager, testRun, null);

      expect(result).toEqual(['scaling-run-1']);
      // Should use LIMIT 1
      expect(mockManager.query).toHaveBeenCalledWith(
    ...
    it('should NOT filter by meetsRequirement in SCALING mode', async () => {
      const testRun = {
        test_run_id: 'scaling-run-2',
        system_under_test_id: 'sut-1',
        workload: 'load-test',
      ...
      // Should NOT include meetsRequirement filter
      expect(mockManager.query).toHaveBeenCalledWith(
        expect.not.stringContaining('meetsRequirement'),
        expect.any(Array)
      );
      // Should NOT include BASELINE mode inclusion
      expect(mockManager.query).toHaveBeenCalledWith(
        expect.not.stringContaining("adapt_config->>'mode' = 'BASELINE'"),
        expect.any(Array)
      );
    });

    it('should respect differencesAccepted filter in SCALING mode', async () => {
      const testRun = {
        test_run_id: 'scaling-run-2',
        system_under_test_id: 'sut-1',
        workload: 'load-test',
      ...
      expect(mockManager.query).toHaveBeenCalledWith(
        expect.stringContaining("differencesAccepted"),
        expect.any(Array)
      );
    });

    it('should respect RBAC organization filter in SCALING mode', async () => {
      const testRun = {
        test_run_id: 'scaling-run-2',
        system_under_test_id: 'sut-1',
        workload: 'load-test',
    ...
    it('should use tuple filter when no explicit baseline', async () => {
      const testRun = {
        test_run_id: 'scaling-run-2',
        system_under_test_id: 'sut-1',
        workload: 'load-test',
      ...
      expect(mockManager.query).toHaveBeenCalledWith(
        expect.stringContaining('system_under_test_id = $1'),
        expect.arrayContaining(['sut-1', 'load-test', 'production'])
      );
    });
  });
