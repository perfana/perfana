...
| Audit-report (real but truncated) | `apps/api/CODE_AUDIT_REPORT.md` (deleted from tree; in history `cb6c7630`→`168bd4f`) | A *prior* security audit that documented secrets found in a developer's local `.env`/`.env.local`. The values are truncated to non-usable fragments: `SUPABASE_SERVICE_ROLE_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9... (LEGACY)`, `DYNATRACE_API_TOKEN=dt0c01.RKQYIWNCWT7TETKTU5F5RSUQ... (EXPOSED API KEY)`. **The secret portions are always `...`-truncated** (verified across all history); only the Dynatrace token *public id* is present. The audited `.env.local` itself was never committed. trufflehog verified 0. → Not usable, but the Dynatrace token is a **real token public-id**, so it is added to the mandatory Task 7 rotation scope as defense-in-depth. |
...
defense-in-depth for the private→public transition. **Rotation scope must
include the Dynatrace API token** (`dt0c01.RKQYIWNCWT7TETKTU5F5RSUQ…`) whose
public id appears truncated in the historical `CODE_AUDIT_REPORT.md`, alongside
the Keycloak client secret, DB passwords, and any Supabase/gstack keys.
