...
- **A dashboard uid is unique only within a Grafana instance.** The same uid routinely exists on several, so the sweep's application-dashboard lookup scopes by `grafana_instance_id`. Without that scope one instance's application dashboards vouch for another instance's copy and the sweep pushes a dashboard into the wrong Grafana. **This rule is not yet applied everywhere** — the API's delete pre-check (`GrafanaDashboardsService.remove`) matches on `dashboardUid` with no instance scope, so a same-uid application dashboard on a different instance can produce a false 409.
