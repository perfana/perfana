...
> [!note] A profile SLO that collides with a manual one is skipped, not fatal
> Since v0.2.96.15 `uq_benchmarks_active_metric_target` (migration 1812) refuses a second **enabled** metric SLO on the same panel, series and aggregation. The provisioning pass's existence probe keys on `generic_check_id`, which a manually-created SLO never has, so a manual SLO already sitting on that panel is invisible to the probe and the collision surfaces as a 23505 from the insert. `insertBenchmarkBasedOnProfileBenchmark` recognises it with `isDuplicateSloTargetError` (`@perfana/shared/utils/duplicate-slo-target`) and returns `skippedDuplicateTarget: true` instead of throwing — rethrowing would abort provisioning for the whole dashboard over one collision, the same shape as the v0.2.89.0 "one rejected dashboard aborts the sweep" bug. Expected during a manual-to-profile cutover; the warning names the panel and says what to do, which is to remove or disable the manual SLO the profile now supersedes.
