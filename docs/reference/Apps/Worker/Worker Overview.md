...
| Service | Purpose |
|---|---|
| `PipelineOrchestrator` | Coordinates 11 pipeline implementations; dispatches the 10 stage names in `ORCHESTRATED_STAGES`. Its `timeoutMs` is a wall-clock race **per stage**, not per pipeline, and since v0.2.95.17 it does not apply to the stages in `HEAVY_STAGES` at all — they are bounded by `statement_timeout` and serialised by `HeavyStageMutex` instead. On a run whose incremental collection was gap-filled it skips `dynatrace-collection`, `panels-processing` and `metrics-collection`, but never `performance-test-metrics` (v0.2.95.22): that pipeline decides for itself (`planFullCollection`, v0.2.95.23) whether to skip, aggregate only the tail after the last tick, or delete and rebuild. `checkAndFillMetricGaps` never marks the perf-test status row complete — its `is_complete` is the stage's own finalisation marker |
| `JobLockService` | Prevents concurrent jobs on same scope (`sut:env:workload`). Since v0.2.95.17 an `analyze-test` job refused by it is re-parked (`moveToDelayed`, 60 s at a time for up to 2 h) rather than retried and dropped. Also exports `perfTestTickLockKey` (v0.2.95.23): the per-run key lock a live perf-test tick holds, which the analyze-time full pass takes (up to 3 min, best-effort) so a tick that started before completion cannot land after it |
| `HeavyStageMutex` | Deployment-wide Redis lock (`SET NX PX`, 5 min TTL, 60 s heartbeat, 1 h give-up) around `statistics-calculation`, `control-group-statistics`, `adapt-analysis` and, since v0.2.95.22, the analyze-time `performance-test-metrics` stage (not the live per-minute tick; since v0.2.95.23 that stage is usually a tail pass, with the full rebuild as fallback), so two multi-minute aggregations never run on Postgres at once. Held by whichever job runs the pipeline: the analyze orchestrator, or the registry processor for a re-evaluate's child job — never the re-evaluate orchestrator itself |
...
| `MetricCollectionGapService` | Detects and fills incomplete collections. `detectGaps` never reports a `performance_test` gap and `calculateCoverage` leaves the perf-test status row out of the average (v0.2.95.22): that source is built from `requests_raw` in this database, never from an external source, and a JMeter-only run reads 100%. `isCollectionComplete` ignores the row too (v0.2.95.23): its `is_complete` is set by the perf-test stage, which runs after the check |
...
These are **worker process** budgets. The heavy aggregation transactions in `StatisticsPipeline` and `ControlGroupStatisticsPipeline` also carry a **database-side** budget, set per transaction by `BasePipelineTypeORM.setAggregationBudget()`: `AGGREGATION_STATEMENT_TIMEOUT_MS` (default 540s) and `AGGREGATION_WORK_MEM` (default 128MB). Since v0.2.95.32 the perf-test tick's two aggregates and its statistics upsert (`helpers/perf-metrics-writer.ts`) run under the same budget in their own transaction via `withAggregationBudget` (`helpers/aggregation-budget.ts`), outside `HeavyStageMutex`. All of these are separate from `ANALYTICS_STATEMENT_TIMEOUT_MS`, which stays a lowerable 120s cap on ordinary analytics reads and no longer applies to the budgeted jobs. The `work_mem` value is charged per hash/sort node, per parallel worker, and per concurrent job — size it against the database host's RAM rather than the worker container's. See [[Environment Variables]].
