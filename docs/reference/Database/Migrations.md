...
| `1807000000000-AddDsPanelsTestRunIndex` | `CREATE INDEX IF NOT EXISTS idx_ds_panels_test_run_dashboard ON ds_panels (test_run_id, application_dashboard_id)`. The table had no index on `test_run_id`, so every incremental Grafana tick, `getDsPanelsByTestRun` and the `DELETE` in `PanelsPipeline` scanned it whole. Plain, not `CONCURRENTLY` — the table is small (~9 k rows on dev) and a concurrent build cannot run inside the migration transaction. `down()` drops it. |
