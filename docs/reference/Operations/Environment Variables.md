...
| `WORKER_ANALYZE_CONCURRENCY` | `2` | Concurrent jobs on the analyze queue. Multiplies the `AGGREGATION_WORK_MEM` peak above. Since v0.2.95.17 the heavy stages (`HEAVY_STAGES`: statistics, control-group statistics, ADAPT and, since v0.2.95.22, the analyze-time `performance-test-metrics` rebuild) are serialised across the deployment by `HeavyStageMutex`, so a second analyze job parks in its slot (shown as **Queued**) while the first aggregates; with holder + parked filling both slots, incremental-collection ticks wait for the heavy stage. Raise to 3 if that shows up as coverage warnings. |
