...
### Task 3a.8: Engineer-facing docs (capabilities pattern + role decision matrix)

The lint rule's error message will tell developers "Use `getCapabilities()` or `@RequiresCapability`. See …". That `…` needs to be a real, useful page when the rule starts firing — otherwise every blocked developer has to figure out the new pattern from the audit log, which is a process artifact, not a how-to. Ships in the Phase 3a PR alongside the rule itself.

**Files:**
- Create: `docs-site/content/Architecture/Capabilities and RBAC.md` — single page covering both how to use the capabilities API + the role-→-capabilities decision matrix.
- Modify: `apps/api/eslint-rules/no-direct-is-global-admin.js` — update the `noDirectIsGlobalAdmin` message to point at the new docs page URL.

- [ ] **Step 1: Write the docs page**

Create `docs-site/content/Architecture/Capabilities and RBAC.md`:
...
- [ ] **Step 2: Update the lint rule's error message to point at the new docs**

Modify `apps/api/eslint-rules/no-direct-is-global-admin.js`'s `noDirectIsGlobalAdmin` message:
...
```bash
# Sanity-check the docs page renders without broken links
ls "docs-site/content/Architecture/Capabilities and RBAC.md"

# Run the lint rule's own spec to verify the message change didn't break test assertions
cd apps/api && npx jest eslint-rules/no-direct-is-global-admin.spec.js
```

```bash
git add "docs-site/content/Architecture/Capabilities and RBAC.md" apps/api/eslint-rules/no-direct-is-global-admin.js
git commit -m "docs(architecture): capabilities pattern + role decision matrix"
```
...
// If perfana has Sentry wired (verify: grep "@sentry" package.json && grep "Sentry.init" apps/api/src):
Sentry?.addBreadcrumb({
  category: 'auth',
  message: 'capability denied',
  level: 'warning',
  data: { capability: requiredCapability, userId: ctx.userId, orgId: orgId ?? null },
});
throw new ForbiddenException(`Missing capability: ${requiredCapability}`);
```

The structured WARN log is the **minimum** and ships unconditionally. The metrics counter and the Sentry breadcrumb are **optional bonuses**, both gated on the relevant infrastructure already existing — DO NOT introduce Prometheus or Sentry just to support these lines. Verification is a one-line grep at execution time:

```bash
grep -l "Sentry.init\|@sentry" apps/api/src package.json 2>/dev/null   # Sentry?
grep -l "MetricsService\|prom-client\|@nestjs/terminus" apps/api/src package.json 2>/dev/null  # Prometheus?
```

If neither shows results, ship the WARN log alone. The point is observability without scope creep.
