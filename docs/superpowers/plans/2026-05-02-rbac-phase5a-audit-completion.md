...
| 19 | `expected-config-change` repository-layer (no-op verification — scope adjusted) | `ExpectedConfigChangeRepository` has zero production callers (exported from `repositories/index.ts`, never injected); every real mutation flows through `TestRunsConfigService` which PR13 already audits. Add a controller-level integration spec asserting the audit row materializes when `TestRunsComparisonController.{create,delete}ExpectedConfigChange` is called, paralleling PR18's `BenchmarksController` spec. No new `auditService.log*` call sites. Allowlist closure deferred to PR20's `POLICY_EXEMPT` repository-layer batch (closing it in PR19 would force per-method `// audit-skip:` workarounds on the unused mutation methods, pre-empting PR20's design) | none — `expected-config-change.repository.ts` moves with PR20's bulk repository-layer batch |
