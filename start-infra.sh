...
#
# Prerequisites:
#   - perfana-demo repo cloned alongside this repo (../perfana-demo)
...
# Check that perfana-demo symlinks resolve
if [ ! -d "infra/grafana/dashboards" ]; then
  echo "ERROR: infra/grafana/dashboards symlink is broken."
  echo "Make sure perfana-demo is cloned at ../perfana-demo"
  exit 1
fi
...
docker compose -f "$COMPOSE_FILE" up --build perfana-migration
MIGRATION_EXIT=$?
...
echo "[6/6] Starting mock services and load testing..."
...
echo "  ./stop-infra.sh                          # Stop infrastructure"
echo ""
